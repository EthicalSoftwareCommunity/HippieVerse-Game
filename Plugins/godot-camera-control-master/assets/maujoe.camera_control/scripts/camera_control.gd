...
onready var camera:Camera = $UniverseCamera
...
func _ready():
	...
	inputController.connect( "inputEventType", self, "handle_incoming_input")

func handle_incoming_input(type, position):
	inputType = type
	if type == InputTouchEventType.TouchDown:
		pass
	elif type == InputTouchEventType.TouchUp:  
		inputController.reset_input()
		print("Touch to ...")
	elif type == InputTouchEventType.HoldTouchDown:
		print("HOLD Touch to ...")
		var object:Node = get_mouse_raycast_object()
		if object != null:
			var controller = object.has_node("InteractionController")
			if controller:
				controller = object.get_node("InteractionController")
				controller.interact(InputTouchEventType.HoldTouchDown)
				inputController.reset_input()
	elif type == InputTouchEventType.HoldTouchUp: 
		inputController.reset_input()
	elif type == InputTouchEventType.SlideStart:  
		inputController.reset_input()
	elif type == InputTouchEventType.Sliding:  
		_mouse_offset = position
	elif type == InputTouchEventType.SlideEnd:  
		inputController.reset_input()
	elif type == InputTouchEventType.ZoomIn:  
		_direction = 1
	elif type == InputTouchEventType.ZoomOut:  
		_direction = -1
	elif type == InputTouchEventType.ZoomOff:
		_direction = 0
...
func _input(event):
		...
		#if inputType == InputController.InputTouchEventType.TouchDown:
		#	movement = true
		#else:
		#	movement = false
...
func _update_movement(delta):
	var _speed = max_speed * acceleration * transform.basis.z * -1 * 50 * _direction;
...
func check_touch_interact_ability() -> bool:
	var object:Node = get_mouse_raycast_object()
	var controller = object.get_node("InteractionController")
	return controller == null
	
func get_mouse_raycast_object() -> Node:
	var space_state = get_world().direct_space_state
	var mouse_pos = get_viewport().get_mouse_position()
	var ray_origin = camera.project_ray_origin(mouse_pos)
	var ray_end = ray_origin + camera.project_ray_normal(mouse_pos) * 2000
	var results =  space_state.intersect_ray(ray_origin, ray_end, [self])
	var result:Node
	if results.size() > 0:
		result = results["collider"]
		print(result)
	return result
