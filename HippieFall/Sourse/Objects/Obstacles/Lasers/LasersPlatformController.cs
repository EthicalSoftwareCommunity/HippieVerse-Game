...
{
	class LasersPlatformController : Obstacle
	{
		...
		private List<LaserController> _laserBarrel = new List<LaserController>();
		...
		private void InitLasers()
		{
			foreach (NodePath laserPath in _laserBarrelPaths)
				_laserBarrel.Add(GetNode<LaserController>(laserPath));
		}
	}
	...
	class LaserPattern
	{
		public struct LaserDirection
		{
			...
			public readonly LasersPlatformController.DirectionType DirectionType;
			public LaserDirection(int from, int to, LasersPlatformController.DirectionType directionType)
			{
				From = from;
				To = to;
